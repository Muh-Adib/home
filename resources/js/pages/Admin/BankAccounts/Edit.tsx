...
export default function BankAccountsEdit({ bankAccount, paymentMethods = [] }: BankAccountsEditProps) {
    const { data, setData, patch, processing, errors } = useForm({
        payment_method_id: bankAccount.payment_method_id || '',
        account_number: bankAccount.account_number || '',
        account_holder: bankAccount.account_holder || '',
        label: bankAccount.label || '',
        visibility_mode: bankAccount.visibility_mode || 'public',
    });

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: 'Rekening Bank', href: '/admin/bank-accounts' },
        { title: 'Edit Rekening', href: '#' },
    ];

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(`/admin/bank-accounts/${bankAccount.id}`);
    };

    return (
        <AdminLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Rekening ${bankAccount.label} - Admin`} />
            ...
            <div className="max-w-2xl mx-auto space-y-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Edit Rekening Bank</h1>
                        <p className="text-gray-500 text-sm">Perbarui informasi rekening bank properti</p>
                    </div>
                </div>
