...
export default function Edit({ plan, statuses, contentTypes, users = [] }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        title: plan.title || '',
        description: plan.description || '',
        target_keywords: plan.target_keywords || [] as string[],
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.content-plans.update', plan.id), {
            onSuccess: () => {
                toast.success('Content plan updated successfully!');
                router.get(route('admin.content-plans.show', plan.id));
            },
            onError: (errors) => {
    ...
    return (
        <AdminLayout>
            <Head title={`Edit: ${plan.title || 'Content Plan'}`} />
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="flex items-center gap-4 mb-8">
                    <Button
                        variant="outline"
                        size="icon"
                        onClick={() => router.get(route('admin.content-plans.show', plan.id))}
                    >
                        <ArrowLeft className="w-4 h-4" />
                    </Button>
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">Edit Content Plan</h1>
                        <p className="text-gray-600 mt-1">
                            Update content plan details
                        </p>
                    </div>
                </div>
