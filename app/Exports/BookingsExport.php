...
use Maatwebsite\Excel\Concerns\ShouldAutoSize; // opsional
...
class BookingsExport implements FromQuery, WithHeadings, WithMapping, WithStyles, WithColumnWidths, WithEvents
{
    protected $filters;
    ...
    public function __construct(array $filters = [])
    {
        ...
        $this->paymentMethods = \App\Models\PaymentMethod::active()->pluck('name')->toArray();

        // Get all users except guests for Created By dropdown
        $this->users = \App\Models\User::where('role', '!=', 'guest')->pluck('name')->toArray();
    ...
    public function map($booking): array
    {
        ...
        // Get payment method name from the latest payment
        $paymentMethodName = $booking->payments->sortByDesc('created_at')->first()?->paymentMethod?->name ?? '';
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if ($highestRow > 1) {
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                $conditional1->getStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getEndColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_RED);
        ...
        // Add data validation for Payment Status column (AC)
        for ($row = 2; $row <= $highestRow; $row++) {
        ...
        // Add data validation for Payment Method column (AD)
        $paymentMethodCount = count($this->paymentMethods);
        $pmLookupRange = "\$BC\$2:\$BC\$" . ($paymentMethodCount + 1);
        for ($row = 2; $row <= $highestRow; $row++) {
            $validation = $sheet->getCell("AD{$row}")->getDataValidation();
            $validation->setType(DataValidation::TYPE_LIST);
            $validation->setErrorStyle(DataValidation::STYLE_INFORMATION);
        ...
        // Add data validation for Created By column (AG)
        $userCount = count($this->users);
        $userLookupRange = "\$BD\$2:\$BD\$" . ($userCount + 1);
        for ($row = 2; $row <= $highestRow; $row++) {
            $validation = $sheet->getCell("AG{$row}")->getDataValidation();
            $validation->setType(DataValidation::TYPE_LIST);
            $validation->setErrorStyle(DataValidation::STYLE_INFORMATION);
            $validation->setAllowBlank(true);
            $validation->setShowInputMessage(true);
    ...
    public function columnWidths(): array
    {
        return [
            ...
            'AD' => 25, // Payment Method
            'AE' => 30, // Internal Notes
            'AF' => 20, // Created At
            'AG' => 25, // Created By
            'AH' => 25, // Verified By
        ];
