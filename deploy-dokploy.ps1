# Dokploy Deployment Script untuk Windows
...
Write-ColorOutput "🚀 Starting Dokploy Deployment with Dockerfile" $Blue
...
function Start-Container {
    ...
    $network = docker network ls --format "table {{.Name}}" | Select-String "homsjogja-network"
    if (-not $network) {
...
function Invoke-Migrations {
    ...
    docker exec $ContainerName php artisan migrate --force 2>$null
    ...
    Write-Success "Migrations and cache completed"
}
...
switch ($Command.ToLower()) {
    ...
    default {
        Write-ColorOutput "Usage: .\deploy-dokploy.ps1 {deploy|build|start|stop|restart|logs|status|test|migrate|cleanup}" $White
        Write-ColorOutput ""
